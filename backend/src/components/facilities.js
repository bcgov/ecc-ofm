...
async function getFacility(req, res) {
  try {
    const operation = `accounts(${req.params.accountId})?$select=accountid,accountnumber,name,telephone1,telephone2,emailaddress1,address1_line1,address1_line2,address1_city,address1_postalcode,address1_stateorprovince,ofm_is_mailing_address_different,address2_line1,address2_line2,address2_city,address2_postalcode,address2_stateorprovince,statecode,statuscode&$expand=ofm_primarycontact($select=ccof_userid,ofm_first_name,ofm_last_name,ofm_portal_role)`
    const response = await getOperation(operation)
    const facility = new MappableObjectForFront(response, FacilityMappings).toJSON()
    facility.primaryContact = new MappableObjectForFront(response.ofm_primarycontact, UserMappings).toJSON()
...
async function getFacilityContacts(req, res) {
  try {
    // TODO Remove some filters
    const operation = `ofm_bceid_facilities?$select=_ofm_facility_value&$expand=ofm_bceid($select=ofm_first_name,ofm_is_expense_authority,ofm_last_name,ofm_portal_role)&$filter=(_ofm_facility_value eq ${req.params.facilityId}) and (ofm_bceid/ofm_is_expense_authority eq true or ofm_bceid/ofm_is_primary_contact eq true) and (ofm_portal_access eq true)`
    const response = await getOperation(operation)
    const contacts = []
