...
export function isValidEndDate({ startDate, endDate }, delimiter = '/') {
  if (startDate && endDate) {
    // Replace the delimiter with '-' to ensure the date strings are in a format that can be parsed by the Date constructor
    const formattedStartDate = startDate.replace(new RegExp(delimiter, 'g'), '-')
    const formattedEndDate = endDate.replace(new RegExp(delimiter, 'g'), '-')
