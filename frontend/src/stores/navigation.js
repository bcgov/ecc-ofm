/* TODO: This file was brought across from STUDENT-ADMIN with some uncertainty as to whether common framework functionality.
   It does appear to have relation to the navbar. Remove file from code base if its purpose was business specific to STUDENT-ADMIN.
*/
    ...
    title: (state) => {
      ...
      if (state.multiFiles) {
        let numberOfSelectedFiles = [
          ...new Set(state.selectedIDs?.map((item) => item[`${state.requestType}ID`]))
        ]?.length
        files = `(${numberOfSelectedFiles} ${
          numberOfSelectedFiles > 1 ? 'files' : 'file'
        } selected)`
      }
      return `Record ${state.currentRequest + 1} of ${
