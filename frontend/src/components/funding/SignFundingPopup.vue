<template>
  <v-container>
    <AppDialog v-model="isDisplayed" title="Signature Required!" persistent max-width="60%" @close="closeDialog">
      <template #content>
        <v-card
          v-for="fundingAgreement in fundingAgreements"
          :key="fundingAgreement.facilityId"
          class="confirm-dialog-text d-flex flex-column mx-md-14 py-6 my-2">
        ...
        <br />
        <h2 class="confirm-dialog-text d-flex flex-column text-center">
          You have one or more Funding Agreements ready to sign.
        </h2>
        <div class="confirm-dialog-text d-flex flex-column text-center">
          Please visit the notification center for more details.
        </div>
      ...
      <template #button>
        <v-row justify="space-around">
          ...
          <v-col cols="12" md="6" class="d-flex justify-center">
            <AppButton
              id="dialog-cancel-application"
              size="large"
              min-width="250px"
              max-width="500px"
              :loading="loading"
              @click="goToFunding">
              Yes, take me to Funding
            </AppButton>
          </v-col>
        </v-row>
      </template>
